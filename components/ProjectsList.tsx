    ...
    <section id={id} className="py-6">
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        {items.map((p, i) => (
          <div
            key={i}
            className="flex flex-col rounded-xl border border-gray-200 dark:border-neutral-800 overflow-hidden bg-gradient-to-br from-gray-400/6 to-transparent dark:from-gray-500/6"
          >
            {p.image ? (
              <div
            ...
            <article className="p-3">
              <div className="flex flex-col sm:flex-col gap-2 h-full">
                <div className="min-w-0 flex-1">
                  ...
                  <p
                    className="mt-1 text-sm text-gray-700 dark:text-neutral-300"
                    style={{
                      display: "-webkit-box",
                      WebkitLineClamp: 3,
                  ...
                  {p.tags && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {p.tags.map((t, k) => (
                        <span
                          key={k}
                          className="rounded-full border border-gray-200 dark:border-neutral-800 px-2 py-0.5 text-[10px] text-gray-700 dark:text-neutral-300"
                        >
                          {t}
                        </span>
                      ))}
